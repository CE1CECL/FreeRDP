 ...
 * RDP6_BITMAP_STREAM
 */
static bolean bitmap_decompress4(uint8* srcData, uint8* dstData, int width, int height, int size)
{
	int RLE;
	int code;
	int NoAlpha;
 ...
 */
bolean bitmap_decompress(uint8* srcData, uint8* dstData, int width, int height, int size, int srcBpp, int dstBpp)
{
        uint8 * TmpBfr;
