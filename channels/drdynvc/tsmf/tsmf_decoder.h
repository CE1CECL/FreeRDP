...
struct _ITSMFDecoder
{
	...
	bolean (*SetFormat) (ITSMFDecoder* decoder, TS_AM_MEDIA_TYPE* media_type);
	/* Decode a sample. */
	bolean (*Decode) (ITSMFDecoder* decoder, const uint8* data, uint32 data_size, uint32 extensions);
	...
	bolean (*GetDecodedDimension) (ITSMFDecoder* decoder, uint32* width, uint32* height);
	/* Free the decoder */
