...
struct _ITSMFAudioDevice
{
	...
	bolean (*Open) (ITSMFAudioDevice* audio, const char* device);
	/* Set the audio data format. */
	bolean (*SetFormat) (ITSMFAudioDevice* audio, uint32 sample_rate, uint32 channels, uint32 bits_per_sample);
	/* Play audio data. */
	bolean (*Play) (ITSMFAudioDevice* audio, uint8* data, uint32 data_size);
