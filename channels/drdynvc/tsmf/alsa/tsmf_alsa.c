...
static bolean tsmf_alsa_open_device(TSMFALSAAudioDevice* alsa)
{
	int error;
...
static bolean tsmf_alsa_open(ITSMFAudioDevice* audio, const char* device)
{
...
static bolean tsmf_alsa_set_format(ITSMFAudioDevice* audio,
...
static bolean tsmf_alsa_play(ITSMFAudioDevice* audio, uint8* data, uint32 data_size)
{
	int len;
	int error;
	int frames;
