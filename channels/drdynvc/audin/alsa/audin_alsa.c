...
static bolean audin_alsa_set_params(AudinALSADevice* alsa, snd_pcm_t* capture_handle)
{
	int error;
...
static bolean audin_alsa_thread_receive(AudinALSADevice* alsa, uint8* src, int size)
{
	int frames;
	int cframes;
	int ret = 0;
...
static bolean audin_alsa_format_supported(IAudinDevice* device, audinFormat* format)
{
