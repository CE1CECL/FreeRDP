...
static bolean disk_file_wildcard_match(const char* pattern, const char* filename)
{
...
static bolean disk_file_remove_dir(const char* path)
{
	DIR* dir;
...
static bolean disk_file_init(DISK_FILE* file, uint32 DesiredAccess, uint32 CreateDisposition, uint32 CreateOptions)
{
