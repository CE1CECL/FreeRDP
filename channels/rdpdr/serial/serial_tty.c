...
static bolean tty_get_termios(SERIAL_TTY* tty);
...
bolean serial_tty_read(SERIAL_TTY* tty, uint8* buffer, uint32* Length)
{
	long timeout = 90;
...
bolean serial_tty_write(SERIAL_TTY* tty, uint8* buffer, uint32 Length)
{
	ssize_t r;
...
bolean serial_tty_get_event(SERIAL_TTY* tty, uint32* result)
{
	int bytes;
	bolean ret = false;
