...
static uint32 sc_output_string(IRP* irp, char *src, bolean wide)
{
	uint8* p;
	uint32 len;
...
static uint32 sc_input_string(IRP* irp, char **dest, uint32 dataLength, bolean wide)
{
	char *buffer;
	int bufferSize;
...
static void sc_input_reader_name(IRP* irp, char **dest, bolean wide)
{
	uint32 dataLength;
...
static uint32 handle_ListReaders(IRP* irp, bolean wide)
{
	uint32 len, rv;
	SCARDCONTEXT hContext;
	DWORD dwReaders;
...
static uint32 handle_GetStatusChange(IRP* irp, bolean wide)
{
	LONG rv;
	SCARDCONTEXT hContext;
	DWORD dwTimeout = 0;
...
static uint32 handle_Connect(IRP* irp, bolean wide)
{
	LONG rv;
	SCARDCONTEXT hContext;
...
static DWORD handle_Status(IRP *irp, bolean wide)
{
	LONG rv;
	SCARDHANDLE hCard;
...
static uint32 handle_LocateCardsByATR(IRP* irp, bolean wide)
{
	LONG rv;
	int i, j, k;
	SCARDCONTEXT hContext;
