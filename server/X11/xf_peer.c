...
extern bolean xf_pcap_dump_realtime;
...
static bolean xf_peer_sleep_tsdiff(uint32 *old_sec, uint32 *old_usec, uint32 new_sec, uint32 new_usec)
{
	sint32 sec, usec;
...
bolean xf_peer_get_fds(freerdp_peer* client, void** rfds, int* rcount)
{
	xfPeerContext* xfp = (xfPeerContext*) client->context;
...
bolean xf_peer_check_fds(freerdp_peer* client)
{
	xfInfo* xfi;
	xfEvent* event;
	xfPeerContext* xfp;
...
bolean xf_peer_capabilities(freerdp_peer* client)
{
	return true;
}

bolean xf_peer_post_connect(freerdp_peer* client)
{
	xfInfo* xfi;
	xfPeerContext* xfp;
...
bolean xf_peer_activate(freerdp_peer* client)
{
	xfPeerContext* xfp = (xfPeerContext*) client->context;
