...
bolean pcap_has_next_record(rdpPcap* pcap)
{
...
bolean pcap_get_next_record_header(rdpPcap* pcap, pcap_record* record)
{
...
bolean pcap_get_next_record_content(rdpPcap* pcap, pcap_record* record)
{
	fread(record->data, record->length, 1, pcap->fp);
	return true;
}

bolean pcap_get_next_record(rdpPcap* pcap, pcap_record* record)
{
	if (pcap_has_next_record(pcap) != true)
		return false;
...
rdpPcap* pcap_open(char* name, bolean write)
{
	rdpPcap* pcap;
