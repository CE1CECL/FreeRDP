...
bolean rdp_recv_synchronize_pdu(rdpRdp* rdp, STREAM* s)
{
...
bolean rdp_recv_server_synchronize_pdu(rdpRdp* rdp, STREAM* s)
{
	rdp->finalize_sc_pdus |= FINALIZE_SC_SYNCHRONIZE_PDU;
	return true;
}

bolean rdp_send_server_synchronize_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_recv_client_synchronize_pdu(rdpRdp* rdp, STREAM* s)
{
	uint16 messageType;
...
bolean rdp_send_client_synchronize_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_recv_control_pdu(STREAM* s, uint16* action)
{
...
bolean rdp_recv_server_control_pdu(rdpRdp* rdp, STREAM* s)
{
	uint16 action;
...
bolean rdp_send_server_control_cooperate_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_send_server_control_granted_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_send_client_control_pdu(rdpRdp* rdp, uint16 action)
{
	STREAM* s;
...
bolean rdp_send_client_persistent_key_list_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_recv_client_font_list_pdu(STREAM* s)
{
	if (stream_get_left(s) < 8)
		return false;
...
bolean rdp_send_client_font_list_pdu(rdpRdp* rdp, uint16 flags)
{
	STREAM* s;
...
bolean rdp_recv_font_map_pdu(rdpRdp* rdp, STREAM* s)
{
	if (rdp->settings->server_mode)
...
bolean rdp_recv_server_font_map_pdu(rdpRdp* rdp, STREAM* s)
{
	rdp->finalize_sc_pdus |= FINALIZE_SC_FONT_MAP_PDU;
	return true;
}

bolean rdp_recv_client_font_map_pdu(rdpRdp* rdp, STREAM* s)
{
	rdp->finalize_sc_pdus |= FINALIZE_SC_FONT_MAP_PDU;
...
bolean rdp_send_server_font_map_pdu(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_recv_deactivate_all(rdpRdp* rdp, STREAM* s)
{
...
bolean rdp_send_deactivate_all(rdpRdp* rdp)
{
	STREAM* s;
...
bolean rdp_server_accept_client_control_pdu(rdpRdp* rdp, STREAM* s)
{
	uint16 action;
...
bolean rdp_server_accept_client_font_list_pdu(rdpRdp* rdp, STREAM* s)
{
