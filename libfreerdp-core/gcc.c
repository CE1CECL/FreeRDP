 ...
 * 	lockedConference		BOLEAN,
 * 	listedConference		BOLEAN,
 * 	conductibleConference		BOLEAN,
...
bolean gcc_read_conference_create_request(STREAM* s, rdpSettings* settings)
{
	uint16 length;
	uint8 choice;
	uint8 number;
...
bolean gcc_read_conference_create_response(STREAM* s, rdpSettings* settings)
{
	uint16 length;
	uint32 tag;
	uint16 nodeID;
...
bolean gcc_read_client_data_blocks(STREAM* s, rdpSettings* settings, int length)
{
	uint16 type;
	uint16 blockLength;
	int pos;
...
bolean gcc_read_server_data_blocks(STREAM* s, rdpSettings* settings, int length)
{
	uint16 type;
	uint16 offset = 0;
	uint16 blockLength;
...
bolean gcc_read_user_data_header(STREAM* s, uint16* type, uint16* length)
{
...
bolean gcc_read_client_core_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	uint32 version;
	uint16 colorDepth = 0;
...
bolean gcc_read_server_core_data(STREAM* s, rdpSettings* settings)
{
	uint32 version;
...
bolean gcc_read_client_security_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	if (blockLength < 8)
		return false;
...
bolean gcc_read_server_security_data(STREAM* s, rdpSettings* settings)
{
	uint8* data;
	uint32 length;
...
bolean gcc_read_client_network_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	int i;
...
bolean gcc_read_server_network_data(STREAM* s, rdpSettings* settings)
{
	int i;
	uint16 MCSChannelId;
	uint16 channelCount;
...
bolean gcc_read_client_cluster_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	uint32 flags;
...
bolean gcc_read_client_monitor_data(STREAM* s, rdpSettings* settings, uint16 blockLength)
{
	printf("CS_MONITOR\n");
	return true;
}
